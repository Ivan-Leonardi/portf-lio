    ...
    data-aos-duration="1000" name='contato' className='w-full md:h-screen bg-[#06050c] flex justify-center items-center p-4 bg-[url(./assets/star.svg)] bg-cover mt-12'>
      <form onSubmit={(e) => (handleSubmit(e))} method='POST' action="https://getform.io/f/1abd8888-5a54-4edb-aad5-b151d32d4c9b" className='flex flex-col max-w-[600px] w-full'>
        <div pb-8>
          ...
          <p className='text-gray-100 py-4'>Não hesite em nos contatar. Deixe sua mensagem e estaremos comprometidos em encontrar soluções.</p>
        </div>

        <input className='p-2 bg-[#EFF8E2] rounded-md outline-none placeholder-gray-400' type="text" placeholder='Nome completo' name='name' onChange={(e) => handleChange(e)} />
        {emptyValues && form.name == "" ? <span className='text-red-500'>O campo nome precisa ser preenchido</span> : ""}

        <input className='my-4 p-2 bg-[#EFF8E2] rounded-md outline-none placeholder-gray-400' type="email" placeholder='Email' name='email' onChange={(e) => handleChange(e)} />
        ...
        <textarea className='bg-[#EFF8E2] p-2 rounded-md outline-none placeholder-gray-400' name="message" rows="10" placeholder='Menssagem' onChange={(e) => handleChange(e)}></textarea>
